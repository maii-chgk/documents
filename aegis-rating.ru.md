...
Раздел переписан для большей читаемости с изменением нумерации подпунктов и перенесён в раздел [A.4](#a4). Старая версия описания доступна по [ссылке](https://github.com/maii-chgk/documents/blob/e2c67c7838650f1b7d17ee1abeb7c27c91fd8107/aegis-rating.ru.md)

### А.4. Балансная система Рейтинга <a name="a4"></a>

<a name="a4_1"></a> **A.4.1.** **Общая схема балансной системы Рейтинга.**

<a name="a4_1_1"></a> **A.4.1.1.** Рейтинг команды меняется на величину бонусов, полученных ею за участие в турнирах (см. <a href="#a4_8">А.4.8</a>). Команда начинает со стартового рейтинга (см. <a href="#a4_11">A.4.11</a>).

<a name="a4_1_2"></a> **A.4.1.2.** Бонус зависит от реально занятого места команды и места, которое она должна была занять в соответствии с прогнозом (см. <a href="#a4_5">A.4.5</a>, <a href="#a4_6">A.4.6</a>, <a href="#a4_7">A.4.7</a>).

<a name="a4_1_3"></a> **A.4.1.3.** Для учёта влияния легионеров и оценки силы сборных команд вычисляется т.н. рейтинг игроков (см. <a href="#a4_9">A.4.9</a>).

<a name="a4_1_4"></a> **A.4.1.4.** Рейтинг игроков используется для расчёта технического рейтинга команды, который определяется её составом на турнире (см. <a href="#a4_10">A.4.10</a>).

<a name="a4_1_5"></a> **A.4.1.5.** Рейтинг игроков и рейтинг команд связаны коэффициентом $Q$, который пересчитывается каждый релиз (см. <a href="#a4_10_2">A.4.10.2</a>) на основе рейтинга команд из Топ-100 и технических рейтингов их базовых составов.



<a name="a4_2"></a> **А.4.2.** Расчёт турнира производится исходя из рейтингов участвующих в нём команд на момент окончания турнира.
...
<a name="a4_5_1_1"></a> **A.4.5.1.1.** Для команды определены:
- $R_T$ - рейтинг по фактическому составу;
- $R_B$ - рейтинг по базовому составу (если у команды есть БС);
- $R$ - релизный рейтинг (если у команды есть БС);
- отношение $R_X = R \times R_T / R_B$.

Если у команды не определён $R_B$ или он нулевой, в качестве $R_X$ используется $R_T$.

<a name="a4_5_1_2"></a> **A.4.5.1.2.** Если рейтинг $R_T$ по фактическому составу меньше рейтинга $R_B$ по базовому составу, турнирный рейтинг $R_{G}$ равен 
максимуму из $R_X$ и $0.5 \times R$.

<a name="a4_5_1_3"></a> **A.4.5.1.3.** Если рейтинг $R_T$ по фактическому составу больше или равен рейтингу $R_B$ по базовому составу, турнирный рейтинг $R_{G}$ равен
минимуму из $R_X$ и максимуму из $R$ и $R_T$.

<a name="a4_5_2"></a> **А.4.5.2.** Список участников турнира упорядочивается по убыванию турнирных рейтингов команд. 

<a name="a4_5_3"></a> **А.4.5.3.** Для каждой команды вычисляются прогнозируемое место $M_P$ (при равенстве рейтингов — среднее арифметическое диапазона мест) и прогнозируемый балл $B_P=CS$, где:
- $S$ — сумма рейтингов 15 команд, занимающих в списке прогнозируемых мест данное место и следующие за ним места, умноженных на коэффициенты уменьшения влияния нижестоящих команд $t_i$ (геометрическая прогрессия с первым членом, равным 1, и знаменателем 0,5: 1, 1/2, 1/4, 1/8…); 
- $C$ — коэффициент, определяемый по последнему перед турниром релизу Рейтинга исходя из возможности получения максимального балла $B_{max}$ на турнире, в котором участвуют 15 сильнейших команд мира (принято $B_{max}=2300$). 
...
<a name="a4_6"></a> **А.4.6.** По результатам турнира для каждой команды определяются занятое место $M$ и турнирный балл $B$ (по формуле, аналогичной описанной в пункте <a href="#a4_5">А.4.5</a>), равный прогнозируемому баллу для места $M$.
...
- $D_2$ — добавочный бонус $D_2=300*\exp((B-2300)/350)$.

Если $D>0$, а число игроков, не входящих в базовый состав $N>2$, то $D$ умножается на $2/N$.

<a name="a4_8"></a> **А.4.8.** Релизный рейтинг каждой команды определяется как сумма рейтинга по предыдущему релизу и всех результатов $D$, полученных на рейтингуемых турнирах. Если релизный рейтинг $R$ оказывается отрицательным, он принимается равным нулю.

<a name="a4_9"></a> **А.4.9.** **Рейтинг игроков.** 

<a name="a4_9_1"></a> **А.4.9.1.** Начиная с сезона 2006/2007 рассчитывается рейтинг игроков $I$ — сумма семи лучших баллов $B$, набранных командами с участием игрока, умноженных на коэффициент учёта турниров $J_i$ (для последней недели равен 1, для предшествующих недель — уменьшается по геометрической прогрессии со знаменателем 0,99).
...
<a name="a4_9_4"></a> **А.4.9.4.** Если ранее исключённый из рейтингования игрок принял участие в турнире или введён в базовый состав команды, то он вносится в пререлиз, предшествующий его участию в турнире или появлению в базовом составе (если игрок внесён в базовый состав на следующий сезон, он вносится в первый пререлиз сезона), при условии, что его рейтинг, рассчитанный на момент этого пререлиза, больше нуля.

<a name="a4_10"></a> **А.4.10.** **Технический рейтинг.** 

<a name="a4_10_1"></a> **А.4.10.1.** Технический рейтинг команды $R_T$ — умноженная на нормирующий коэффициент $Q$ сумма рейтингов шести наиболее высокорейтинговых игроков, отсортированных по убыванию рейтинга с коэффициентами $1$, $5/6$, $2/3$, $1/2$, $1/3$, $1/6$. 

<a name="a4_10_2"></a> **А.4.10.2.** Коэффициент $Q$ вычисляется при релизе как среднее значение отношения рейтинга $R$ к техническому рейтингу по базовому составу RB для команд, входящих в 100 лучших по последнему релизу (исключая те, которые получают в этом релизе стартовые рейтинги) и имеющих не менее шести игроков в базовом составе.

<a name="a4_11"></a> **А.4.11.** **Стартовый рейтинг.** 

<a name="a4_11_1"></a> **А.4.11.1.** Начиная с сезона 2006/2007 новые и восстановленные после промежутка неактивности команды получают стартовый рейтинг на основе базового состава (технический рейтинг по базовому составу RB, умноженный на понижающий коэффициент $G=0,8$). 

<a name="a4_11_2"></a> **А.4.11.2.** Стартовый рейтинг вычисляется также для команд, потерявших преемственность к прошлому сезону (в момент разрыва преемственности), и команд, текущий рейтинг которых оказался ниже вычисленного стартового (при формировании базового состава или в результате внесения в базовый состав новых игроков). 

<a name="a4_11_3"></a> **А.4.11.3.** Стартовый рейтинг вносится в пререлиз, предшествовавший формированию базового состава на сезон; если команда заявила базовый состав до начала сезона — в первый пререлиз сезона.

**Примечание:** для сезона 2006/07 все базовые составы считаются сформированными на 1 сентября 2006 года.

<a name="a4_11_4"></a> **А.4.11.4.** Исходный список рейтинг-листа от 1 июня 1998 года сформирован на основе списка команд «саранского» рейтинга 1998 года. В дальнейшем до 1 сентября 2006 новые команды стартовали с нулевыми рейтингами.
