...
<a name="a30"></a> **A.3.0.** Общая схема балансной системы Рейтинга.

<a name="a301"></a> **A.3.0.1.** Рейтинг команды меняется на величину бонусов, полученных ею за участие в турнирах (см. <a href="a37">А.3.7</a>). Команда начинает со стартового рейтинга (см. <a href="a312"></a>).

<a name="a302"></a> **A.3.0.2.** Бонус зависит от реально занятого места команды и места, которое она должна была занять в соответствии с прогнозом (см. <a href="a34">A.3.4</a>, <a href="a35">A.3.5</a>, <a href="a36">A.3.6</a>).

<a name="a303"></a> **A.3.0.3.** Для учёта влияния легионеров и оценки силы сборных команд вычисляется т.н. рейтинг игроков (см. <a href="a310">A.3.10</a>).

<a name="a304"></a> **A.3.0.4.** Рейтинг игроков используется для расчёта технического рейтинга команды, который определяется её составом на турнире (см. <a href="a311">A.3.11</a>).

<a name="a305"></a> **A.3.0.5.** Рейтинг игроков и рейтинг команд связаны коэффициентом $Q$, который пересчитывается каждый релиз (см. <a href="a3112">A.3.11.2</a>) на основе рейтинга команд из Топ-100 и технических рейтингов их базовых составов.
...
**A.3.4.0.** Турнирный рейтинг $R_G$ команды.

**A.3.4.0.1.** Для команды определены:
...
**A.3.4.0.2.** Если рейтинг $R_T$ по фактическому составу меньше рейтинга $R_B$ по базовому составу, турнирный рейтинг $R_{G}$ равен 
максимуму из $R_X$ и $0.5 \times R$.

**A.3.4.0.3.** Если рейтинг $R_T$ по фактическому составу больше или равен рейтингу $R_B$ по базовому составу, турнирный рейтинг $R_{G}$ равен
...
**А.3.4.2.** Для каждой команды вычисляются прогнозируемое место $M_P$ (при равенстве рейтингов — среднее арифметическое диапазона мест) и прогнозируемый балл $B_P=CS$, где:
...
**А.3.4.3.** Если несколько команд имеют одинаковый рейтинг и заняли место $x$, то есть разделили места от $x-L$ до $x+L$, то для каждой из этих команд прогнозируемый балл $B_P$ определяется как среднее арифметическое от прогнозируемых баллов, начисленных на места этого диапазона.

<a name="a35"></a> **А.3.5.** По результатам турнира для каждой команды определяются занятое место $M$ и турнирный балл $B$ (по формуле, аналогичной описанной в пункте <a href="a342">А.3.4.2</a>), равный прогнозируемому баллу для места $M$.

<a name="a36"></a> **А.3.6.** Результат турнира для команды $D=U(D_1+D_2)$, где:
...
**А.3.11.1.** Технический рейтинг команды $R_T$ — умноженная на нормирующий коэффициент $Q$ сумма рейтингов шести наиболее высокорейтинговых игроков, отсортированных по убыванию рейтинга с коэффициентами $1$, $5/6$, $2/3$, $1/2$, $1/3$, $1/6$. 
...
<a name="a312"></a> **А.3.12.** Стартовый рейтинг. 

**А.3.12.1.** Начиная с сезона 2006/2007 новые и восстановленные после промежутка неактивности команды получают стартовый рейтинг на основе базового состава (технический рейтинг по базовому составу RB, умноженный на понижающий коэффициент $G=0,8$). 

**А.3.12.2.** Стартовый рейтинг вычисляется также для команд, потерявших преемственность к прошлому сезону (в момент разрыва преемственности), и команд, текущий рейтинг которых оказался ниже вычисленного стартового (при формировании базового состава или в результате внесения в базовый состав новых игроков). 

**А.3.12.3.** Стартовый рейтинг вносится в пререлиз, предшествовавший формированию базового состава на сезон; если команда заявила базовый состав до начала сезона — в первый пререлиз сезона.

**Примечание:** для сезона 2006/07 все базовые составы считаются сформированными на 1 сентября 2006 года.

<a name="a38"></a> **А.3.13.4.** Исходный список рейтинг-листа от 1 июня 1998 года сформирован на основе списка команд «саранского» рейтинга 1998 года. В дальнейшем до 1 сентября 2006 новые команды стартовали с нулевыми рейтингами.
