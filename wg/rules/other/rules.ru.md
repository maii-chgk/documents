...
При проведении [мультиязычных турниров](#26) в контексте данных правил все языковые версии вопроса считаются одним вопросом с общими критериями оценки правильности ответов.
...
При проведении [мультиязычных турниров](#26) в редакционную группу включаются также переводчики отдельных языковых версий пакета.
...
Апелляции на ошибку в переводе вопроса принимаются только на [мультиязычных турнирах](#26) и только если это предусмотрено регламентом турнира.
...
## 2.2. Строго синхронный турнир<a name="22"></a>ём
...
## 2.6. Мультиязычный турнир<a name="26"></a>

Мультиязычный турнир — разновидность турнира, участники которого на различных площадках могут играть различные языковые версии пакета вопросов. Как следует из определения, мультиязычным может быть лишь турнир, проводимый на более чем одной площадке — [строго синхронный турнир](#22), [заочный турнир](#23), [строго синхронный онлайн-турнир](#24) либо [заочный онлайн-турнир](#25). При этом каждый отыгрыш на отдельно взятой площадке проводится на одной языковой версии пакета.

При подготовке пакета вопросов мультиязычного турнира должны выполняться следующие требования:
- доступная командам перед отыгрышем информация должна явно указывать, какая языковая версия пакета является основной и какие языковые версии являются переводными;
