...
<br>
1.6.4. [Апелляция на ошибку в переводе вопроса](#164)
<br>
1.6.5. [Порядок рассмотрения апелляций](#165)
<br>
...
2. [Виды игр](#2)
<br>
2.2. [Формы проведения](#21)
<br>
2.1.1. [Очный турнир](#211)
<br>
2.1.2. [Строго синхронный турнир](#212)
<br>
2.1.3. [Заочный турнир](#213)
<br>
2.1.4. [Строго синхронный онлайн-турнир](#214)
<br>
2.1.5. [Заочный онлайн-турнир](#215)
<br>
2.2. [Мультиязычный турнир](#22)
<br>
...
При проведении [мультиязычных турниров](#22) в контексте данных правил все языковые версии вопроса считаются одним вопросом с общими критериями оценки правильности ответов.
...
При проведении [мультиязычных турниров](#22) в редакционную группу включаются также переводчики отдельных языковых версий пакета.
...
### 1.6.4. Апелляция на ошибку в переводе вопроса<a name="164"></a>

Апелляции на ошибку в переводе вопроса принимаются только на [мультиязычных турнирах](#22) и только если это предусмотрено регламентом турнира.
...
### 1.6.5. Порядок рассмотрения апелляций<a name="165"></a>
...
## 2.1. Формы проведения<a name="21"></a> 

## 2.1.1. Очный турнир<a name="211"></a>
...
## 2.1.2. Строго синхронный турнир<a name="212"></a>
...
## 2.1.3. Заочный турнир<a name="213"></a>
...
## 2.1.4. Строго синхронный онлайн-турнир<a name="214"></a>
...
## 2.1.5. Заочный онлайн-турнир<a name="215"></a>
...
## 2.2. Мультиязычный турнир<a name="22"></a>

Мультиязычный турнир — разновидность турнира, участники которого на различных площадках могут играть версии пакета вопросов на разных языках. Мультиязычным может быть лишь турнир, проводимый более чем на одной площадке — [строго синхронный турнир](#212), [заочный турнир](#213), [строго синхронный онлайн-турнир](#214) либо [заочный онлайн-турнир](#215). При этом каждый отыгрыш на отдельно взятой площадке проводится на одной языковой версии пакета.

При подготовке пакета вопросов мультиязычного турнира должны выполняться следующие требования:
- доступная командам перед отыгрышем информация должна явно указывать, какая из языковых версий является основной, а какие — переводными;
- редакционная группа готовит основную языковую версию пакета;
- переводчики в сотрудничестве с редакторами основной версии готовят переводные языковые версии пакета;
- при оформлении основной языковой версии должно быть явно указано, что это основная (оригинальная) версия, и на каком языке она написана;
- при оформлении переводных языковых версий должно быть явно указано, что это переводная версия, на какой язык она переведена, и список переводчиков.
...
Под функцинальной эквивалентностью подразумевается, что оригинал и перевод содержат одни и те же приёмы, заложенны редакторами, такие как смысловые замены, смысловые подсказки, технические отсечки указанием на число слов в ответе и т.п. Переводная версия не должна ни упускать, ни добавлять новые подсказки или способы нахождения ответа. В частности, этот пункт означает, что редакторам и переводчикам следует максимально учитывать особенности всех языков, на которые переводится пакет, в части таких приёмов, как:
...
- указания на фонетические и графемные особенности ответа (слова, начинающиеся на одну букву, парные согласные и т.д.) — за исключением редких ситуаций, когда это свойство гарантированно выполняется для всех языковых версия — например, в явном виде указано, что ответ следует дать на латыни или на английском языке;
- и т.п.

В общем случае редакторам рекомендуется отказаться от подобных подсказок в пакетах, предназначенных для перевода, и использовать их с максимальной осторожность и только в случае необходимости.
